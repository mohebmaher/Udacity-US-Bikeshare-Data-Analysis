...
# Declare a CONSTANT variable with path to each dataset
...
def load_data():
    ...
    print("Please, choose on of the following datasets:")
    print("  1. Chicago,")
    print("  2. New York,")
    print("  3. Washington.")
    ...
    while True:
        ...
        if not user_input:
            ...
            print("\nLoading data..")
            path = f"./data/{CITIES[user_input]}"
            loaded_data = pd.read_csv(path, parse_dates=['Start Time', 'End Time']).iloc[:, 1:]
            print("Done!")
            return loaded_data
        else:
...
def explore_data(data):
    ...
    while True:
        # Get the user input
        rows = input('Number of rows (to skip press enter): ').strip()
...
def filter_data(data):
    ...
    while True:
        ...
        user_filters = input("Filter by: ").strip().title().split(' ')
...
def time_stats(data):
    ...
    print(f"\nMost common travel month: {most_common_month},")
...
def station_stats(data):
    ...
    print(f"\nMost common start-end combination: {most_common_start}.")
...
def trip_duration_stats(data):
    ...
    print(f"\nMean travel time: {round(mean_travel_time, 2)} minutes.")
...
def main():
    ...
    while True:
        ...
        to_repeat = input("Yes or No: ").strip().lower()
        if to_repeat == 'no':
            break
